...
function num(id){let n=parseFloat(document.getElementById(id).value);if(!Number.isFinite(n)||n<0){alert("Please enter valid numbers (0 or greater).");return null}return n}
...
function calculateStanding(){let d=num("stDbh"),h=num("stHeight"),f=num("stForm");if([d,h,f].some(x=>x===null))return;let ba=Math.PI*(d/2)**2,v=ba*h*f;document.getElementById("stResult").innerHTML=`Result: <strong>${fmt(v)} m³</strong><br>Basal area: ${fmt(ba)} m²`}
...
function clearTimberForm(p){let ids=p==="bf"?["bfThickness","bfWidth","bfLength"]:p==="st"?["stDbh","stHeight"]:["rfD1","rfD2","rfLength"];ids.forEach(id=>document.getElementById(id).value="");if(p==="st")document.getElementById("stForm").value=".45";document.getElementById(p==="bf"?"bfResult":p==="st"?"stResult":"rfResult").textContent="Result: —"}
// ============================================
// KEYBOARD SUPPORT
// ============================================
